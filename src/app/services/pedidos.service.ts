...
export class PedidosService {
  private baseUrlPedido = 'http://chaninfoodapirest-env-1.eba-qpcjtrfp.us-east-2.elasticbeanstalk.com/api/pedidos/search/findByRestauranteIdAndFechaGreaterThanAndEntregado?';
  private baseUrlDetallePedido = 'http://chaninfoodapirest-env-1.eba-qpcjtrfp.us-east-2.elasticbeanstalk.com/api/detallePedidos/search/findByPedidoId?';
  ...
  getPlatillo(idDetalle: number): Observable<Platillo>{
    const urlPlatillo = `http://chaninfoodapirest-env-1.eba-qpcjtrfp.us-east-2.elasticbeanstalk.com/api/detallePedidos/${idDetalle}/platillo`;
    return this.httpClient.get<Platillo>(urlPlatillo);
  }

  pedidoEntregado(pedido: Pedido): Observable<Pedido> {
    const urlPedido = `http://chaninfoodapirest-env-1.eba-qpcjtrfp.us-east-2.elasticbeanstalk.com/api/pedidos/${pedido.id}`;
    return this.httpClient.put<Pedido>(urlPedido, pedido , { headers });
  }

  getPedido(idPedido: number): Observable<Pedido> {
    const urlPedido = `http://chaninfoodapirest-env-1.eba-qpcjtrfp.us-east-2.elasticbeanstalk.com/api/pedidos/${idPedido}`;
